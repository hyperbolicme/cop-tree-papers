...
\begin{document}
% \title{}
% \author{N.S. Narayanaswamy \inst{1}, Anju Srinivasan \inst{2}}
...
\runningtitle{Tree Path Labeling of Path Hypergraphs}
\runningauthors{N.S. Narayanaswamy, Anju Srinivasan}

\author{N.S. Narayanaswamy , Anju Srinivasan }

\affiliation{
  Indian Institute of Technology Madras,  Chennai\\ 
  \email{swamy@cse.iitm.ernet.in, anjuzabil@gmail.com}
}  
...
cardinalities are preserved, i.e. the path labeling is an ICPPL.  
The
...
constructive approaches refine an ICPPL iteratively, such
...
bijections from $supp(\cF)$ to $supp(\cP)$.  First we
present Algorithm \ref{perms} and prove its correctness.  This
...
\noindent
As a result of  algorithm \ref{perms} each leaf of $T$ is present in at most
one path assigned to some set in $\cF$.  In Algorithm \ref{leafasgn} we
identify elements in $supp(\cF)$ whose images are leaves in feasible path labeling
if one exists.
Let vertex $v \in T$ be the unique
...
leaves in $T$, all path images in the
