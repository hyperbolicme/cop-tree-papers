...
%% STACS %% \documentclass[a4paper,UKenglish,numberwithinsect]{lipics} 
...
%% STACS %% \bibliographystyle{plain} %% STACS %%
  % the recommended bibstyle



\documentclass{CRPITStyle} %% CATS %% 
%\documentclass{llncs} %% CATS %% 
%\usepackage{epsfig}   % Packages to use if you wish %% CATS %%
%\usepackage{lscape}   % %% CATS %%
\usepackage[authoryear]{natbib}%% CATS %%
\renewcommand{\cite}{\citep}%% CATS %%
\pagestyle{empty}%% CATS %%
\thispagestyle{empty}%% CATS %%
...
%% UNCOMMENT FOR CATS %%
\newtheorem{theorem}{Theorem}[section] %% CATS %%
\newtheorem{lemma}[theorem]{Lemma} %% CATS %%
\newtheorem{proposition}[theorem]{Proposition} %% CATS %%
\newtheorem{corollary}[theorem]{Corollary} %% CATS %%
\newenvironment{proof}[1][Proof]{\begin{trivlist} %% CATS %%
\item[\hskip \labelsep {\bfseries #1}]}{\end{trivlist}} %% CATS %%
\newtheorem{definition}{Definition}[section] %% CATS %%
\newcommand{\qed}{\hfill \mbox{\raggedright \rule{.07in}{.1in}}} %% CATS %%
...
\affiliation{
  ...
  Indian Institute of Technology Madras, Chennai - 600036\\ [.1in]
  {\tt $^1$swamy@cse.iitm.ernet.in, $^2$asz@cse.iitm.ac.in,
    $^3$anjuzabil@gmail.com} }
%%%%%%%%%%%%%%%%%%%%%%%%%% CATS TITLE END %%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\xnoindent In this paper, the set $\F \subseteq (2^{U} \setminus
...
\xnoindent The {\em intersection graph} \change[AS]{$\bI(\cF)$ of a
  ...
  corresponding sets}{$\bI(\cH)$ of a hypergraph $\cH$ is a graph such
  that its vertex set has a bijection to (the set of hyperdeges of)
  $\cH$ and there exists an edge between two vertices iff their
  corresponding hyperedges} have a non-empty intersection
\cite{mcg04}.

\xnoindent
Two hypergraphs $\cH$, $\cK$ are said to be isomorphic {\em (hypergraph
isomorphism)}
...
hyperedge in $\cK$ where $K = \{\phi(x) \mid x \in H\}$.  

\xnoindent If $\H_\cF \cong \H_\cP$ where $\cP$ is a path system, then
$\H_\cF$ is called a {\em path hypergraph} and $\cP$ is called {\em
  path representation} of $\H_\cF$. If isomorphism is $\phi:
supp(\H_\cF) \rightarrow supp(\H_\cP)$, then it is clear that there is
an induced path labeling $l_\phi: \cF \rightarrow \cP$ to the set
system. In the rest of the document, we may use $\cF$ and/or $\cH_\cF$
interchangeably to refer the set system and/or its hypergraph.

\note[AS]{this paragraph is not lucid enough.}
\xnoindent A graph $G$ that is isomorphic to the intersection graph
$\bI(\cP)$ of a path system $\cP$ of $T$, is a {\em path graph}. This
isomorphism gives a bijection $\cl': V(G) \rightarrow \cP$\remove[AS]{ and is
called a path labeling of $G$}. Moreover, for the purposes of this
...
% \change[AS]{ An {\em overlap graph} $\bO(\F)$ of a set system $\cF$ is
%   a graph such that its vertex set has a bijection to $\cF$ and there
%   exists an edge between two vertices iff their corresponding sets
%   overlap. Two sets $A$ and $B$ are said to overlap, denoted by $A
%   \overlap B$, if they have a non-empty intersection and neither is
%   contained in the other i.e. $A \overlap B \text{ iff } A \cap B \ne
%   \emptyset, A \nsubseteq B, B \nsubseteq A$. Thus $\bO(\cF)$ is a
%   subgraph of $\bI(\F)$ and not necessarily connected. Each connected
...
The following lemma is useful in characterizing feasible tree path labelings.  The
...
\begin{lemma}
  ...
  \cl(S_3))|$.
\end{lemma}
\note[AS]{Following lemma not required. Deleted.} \remove[AS]{
...
\ref{leafasgn}. \note[AS]{next three sentences can be clubbed/the
  annote is pretty much clear} One direction of this claim is clear: that if a path
...
has \annote[AS]{no premature exit condition}{it does. verify.} hence
any input will go through it. Algorithm \ref{leafasgn} has an exit
condition at line \ref{xempty}. \annote[AS]{It can be easily verified
  that $X$ cannot be empty if $\cl$ is a feasible path
  labeling.}{Maybe this needs to be expanded in the appendix} The
reason is that a feasible path labeling has an associated bijection
between $supp(\cF)$ and $V(T)$ i.e. $supp(\cF^{\cl})$ such that the
sets map to paths, ``preserving'' the path labeling. The rest of the
section is devoted to constructively proving that it is sufficient for
a path labeling to be an ICPPL and pass the two filtering algorithms.
To describe in brief, the constructive approaches refine an ICPPL
iteratively, such that at the end of each iteration we have a
``filtered'' path labeling, and finally we have a path labeling that
defines a family of bijections from $supp(\cF)$ to $V(T)$
i.e. $supp(\cF^{\cl})$.\note[AS]{one sentence for each filter here
...
considering pairs of paths that share a leaf.
...
%   \begin{enumerate}[]
%   \item {\invb} {\em I:} $\cl_j(R)$ is a path in $T${\hfill\invb}\\
%     {\invb\hfill} for each $R \in \cF_j${\invb}
%   \item {\invb} {\em II:} $|R| = |\cl_j(R)|${\hfill\invb}\\
%     {\invb\hfill} for each $R \in \cF_j${\invb}
%   \item {\invb} {\em III:} $|R \cap R'| = |\cl_j(R) \cap \cl_j(R')|${\hfill\invb}\\
%     {\invb\hfill} for any two $R, R' \in \cF_j${\invb}
%   \item {\invb} {\em IV:} $|R \cap R' \cap R''|=|\cl_j(R) \cap \cl_j(R') \cap
%     \cl_j(R'')|$ {\hfill\invb}\\
%     {\invb\hfill} for any three, $R, R', R'' \in \cF_j${\invb}
%   \end{enumerate}
...
\begin{lemma}
  ...
  feasible, then the algorithm executes without a premature exit.
\end{lemma}
\begin{proof}
  A premature exit occurs iff the exit condition in line
  \ref{ln:3waycheck}, i.e. failure of three way intersection
  preservation, becomes true in any iteration of the {\em \bf while}
  loop. Since $(\cF,\cl)$ is feasible, there exists a hypergraph
  isomorphism $\phi: supp(\cF) \rightarrow supp(\cF^{\cl})$ such that
  $\phi(x) = v$. Clearly, $\phi$ is a renaming of vertices in
  hypergraph $\cF$ to those in hypergraph $\cF^\cl$. Thus the
  following facts can be observed in every iteration of the loop.
  ...
  \item element $x$ is exclusive in a hyperedge in $\cF$ since
    $v$ is exclusive in a hyperedge in $\cF^\cl$.
  \end{enumerate}
...
%   \end{enumerate}
%   \qed
%   \note[E2]{remove the induction proof. just text saying x and v are
...
\begin{proof}
  ...
  cannot be empty if input is ICPPL and feasible.  
\qed
\end{proof}
...
\begin{proof}
  ...
  \change[AS]{becomes a single path after which ICPIA algorithm is
    used to find the final subset (interval assignment) that
    exhausts}{has no nodes at which point the rest of
    the bijection is obvious, thus completing} $\phi$. This is the
...
value $d$ such that \change[AS]{$T_d$ is a path}{$T_d$ has no
  nodes}. From the lemma \ref{lem:invar1} and \ref{lem:invar3} we know
that $(\cF_d, \cl_d)$ is an ICPPL. \remove[AS]{We also know that the
  special case of ICPPL when the tree is a path is the interval
  assignment (ICPIA) problem.  We now run the ICPIA algorithms
  |nsnrs09| on $(\cF_d, \cl_d)$.}

\xnoindent \remove[AS]{ It is true that $T_d$ is not precisely an
  interval in terms of consecutive integers because they could be
  arbitrarily named nodes a tree. However, it is easy to see that the
  nodes of $T_{d}$ can be ordered from left to right and ranked to get
  intervals $I_i$ for every path $S_i \in \cF_d$ as follows: $I_i =
  \{[l,r] \mid l = \text{ the lowest rank of the nodes in }\cl_d(S_i)
  \text{, } r = l+|\cl_d(S_i)|-1 \}$. Thus we define an interval
  assignment $\cA = \{ (S_i, I_i) \mid S_i \in \cF_d\}$ which is an
  ICPIA and also in the format ICPIA algorithm requires. The ICPIA
  algorithms give us the bijection $\phi_{d+1} : U_d \rightarrow
  V[T_d]$.}  Now we have exactly one bijection $\phi_j$,
\change[AS]{$j \in [d+1]$}{$j \in [d]$} defined for every element $x
...
\remove[AS]{As we will see in section |sec:decompos|, the problem
  ...
  input tree.} When the given tree is restricted to a smaller class,
...
\xnoindent The outline of the algorithm is as follows. \note[E2]{TBD
  observation: T[V-c] is a collection of independent paths. c is the
  center.}  Notice that the path between a leaf $v$ and $r$ has the
...
\item {\em 1/ unlabeled contained hyperedges}: The hyperedges that are
  \note[AS]{we cannot associate disjoint sets to a particular
    ray. since it is not clear where it would be assigned to at this
    point in the algorithm. changed.}  \change[AS]{either contained in
    or disjoint from}{contained in} type 0 hyperedges of $R_i$.\\
...
encountered \annote[E2]{in every call to the saturation of rays
procedure}{TBD: not clear}.
...
$B_i$\note[E2]{TBD: emphasize $s$th iteration}. Now we try to make a
...
%  &  \change[AS]
%   {|supp( \{ P \mid P \text{ is a path from } R_i\\
...
% \begin{table}[h]
%   \begin{tabular}[h]{ll}
%     $P'_i \subseteq R_i$ & such that $r \in P'_i$,\\
%     $|P'_i|$&$ = k + 2 - $ \\
%     &  $|supp( \{ P \mid P$ is a path from $R_i$\\
%     &assigned to type 0 hyperedges $\})|$\\
%     $P''_i$ &$\in \{P_{i,j} \mid j \in [p], j \ne i\}$ \\
...
\begin{algorithm}[h]
  \caption{Algorithm (main subroutine) to find an ICPPL $\cl$ for a
    hypergraph, $\cO$ from $k$-subdivided star graph $T$:
...
\WHILE {$\cL \ne \cO$}
  \STATE $saturate\_$ $rays\_$ $and\_$ $find\_$ $crossing\_$ $hyperedges(\cO, T)$
  \COMMENT {Saturate all rays of $T$ by using algorithm 
  \ref{al:icppl-saturate_rays_symm_starlike}. This subroutine also
  finds the crossing hyperedge $\cB_i$ of each ray $i$. A crossing
  hyperedge is one that needs to be labeled to a path that has
  vertices from exactly two rays.}

  \STATE $partial\_$ $path\_$ $labeling\_$ $of\_$ $crossing\_$ $hyperedges(\cO, T)$
  \COMMENT {Find path labeling of crossing hyperedges by using
...
\STATE Let $\{ v_i
\mid i \in [p], p \text{ is number of leaves of $T$} \}$ 
...
\STATE \COMMENT { $H_{C-icpia-max}$ does not exist for at least one ray. Labeling
  could still be possible because $H_{C-max}$ could be a viable crossing
...
\note[E2]{TBD: find appropriate name for subroutine - |al:icppl-saturate_rays_symm_starlike|} 
\begin{algorithm}[h]
\caption{$saturate\_$ $ rays\_$ $ and\_$ $ find\_$ $ crossing\_$ $ hyperedge(\cO, T)$}
\label{al:icppl-saturate_rays_symm_starlike}
%{\tiny
\begin{algorithmic}[\lndisplay]
 
\STATE Variable $\cB_i$ shall store the crossing hyperedge for $i$th ray. Init
variables: for every $i \in [p]$, $\cB_i \leftarrow \emptyset$
...
\STATE \COMMENT {for each ray}
  ...
  \WHILE {$\cB_i = \emptyset$ and there exists $H \in \cO \setminus \cL$, such that $H$ overlaps with some
   ...
   \ELSE
      ...
      \STATE Continue \COMMENT{Found the crossing hyperedge for this ray; move on to
      next ray}
    \ENDIF    
...
\begin{algorithm}[h]
\caption{$partial\_$ $ path\_$ $ labeling\_$ $ of\_$ $ crossing\_$ $ hyperedges(\cO, T)$}
\label{al:icppl-partial_labeling_symm_starlike}
%{\tiny
\begin{algorithmic}[\lndisplay]

\STATE \COMMENT {Process equal crossing hyperedges. At this point for
  all $i \in [p]$, $\cB_i \ne \emptyset$.} 
\FOR {every $i \in [p], \cB_i \ne \emptyset$}
  \FOR {every $j \in [p]$}
    \IF {$\cB_i = \cB_j$}
      ...
      {Reset crossing hyperedges for $i$th and $j$th rays}
    \ENDIF
  \ENDFOR % j > i
\ENDFOR % i

\STATE \COMMENT {Process intersecting crossing hyperedges}
\FOR {every $i \in [p], \cB_i \ne \emptyset$}
  \FOR {every $j \in [p]$}
...
\note[AS]{removed matrix theory of path labeling section}
%%%%%%%%%%%%%%%%%%%%%% MATRIX THEORY OF PATH LABELING %%%%%%%%%%%%%%%%%%%
%\section{
\remove[AS]{ Finding an assignment of tree paths to a set
  system}
%}
% \label{sec:decompos} 
...
% \xnoindent An {\em in-tree} is a directed rooted tree in which all
% edges are directed toward to the root.
%% END FROM PRELIMS

% \note[AS]{MOVED FROM KSTAR SECTION} \add[AS]{Algorithm
...
% \note[AS]{MOVED FROM KSTAR SECTION} \add[AS]{Following the notation
...
% \note[AS]{MOVED FROM KSTAR SECTION}
...
\begin{proof}[Proof of lemma \ref{lem:invar3} (alternate)]
  ...
  \item {\em Invariant} IV
    ...
    \item {\em At least two of $R, R', R''$ are new sets.}  If two or
      ...
      \note[AS]{The following is not correct. While loop only
